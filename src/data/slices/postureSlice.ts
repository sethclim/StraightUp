...
type PostureActions = {
    ...
    updateNeckInclination: (value: number) => void;
};
...
> = (set, get) => {
    return {
        ...
        updatePostureStatus(status) {
            ...
            if (oldStatus !== status) {
                ...
                postureTimer = setInterval(() => {
                    set((state) => ({
                        postureDuration: state.postureDuration + 1,
                    }));
                }, 100);
            }
        },
        ...
        toggleShouldRenderLandmarks: () => {
            set((state) => ({
                shouldRenderLandmarks: !state.shouldRenderLandmarks,
            }));
            console.log(get().shouldRenderLandmarks);
        },
        updateNeckInclination: (value: number) => {
            set({ neckInclination: value });
        },
    };
};
